...
static const char *GLOBAL_BRIGHTNESS_KEY = "brightness";
...
static void gfx_draw_loop(void *args)
{
    ...
    {
        uint32_t color = gfx_rgb_set_brightness(output_buffer[i], global_brightness / 255.0);

        if (led_strip_set_pixel(led_strip_handle, i, (color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF) != ESP_OK)
        {
...
static void gfx_config_update_handler(void *args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    if (strcmp(event->namespace, NAMESPACE) == 0 && id == CONFIG_MANAGER_EVENT_UPDATE)
    {
        if (strcmp(event->key, TRANSITION_DURATION_KEY) == 0)
        {
            update_transition_duration();
        }
        else if (strcmp(event->key, GLOBAL_BRIGHTNESS_KEY) == 0)
        {
            if (config_manager_get_u8(NAMESPACE, GLOBAL_BRIGHTNESS_KEY, &global_brightness) != ESP_OK)
            {
                ESP_LOGW(TAG, "Failed to get global brightness");
            }
        }
    }
}
